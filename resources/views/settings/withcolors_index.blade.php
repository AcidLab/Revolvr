...
<div class="row">
    ...
    <div class="col-md-2 offset-md-10"  style="text-align: center;">
        <a href="{{$create_route}}" class="btn btn-primary btn-icon-split" style="width:100%;">
...
<div class="table-responsive">
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                           @foreach($tableOfObjects as $row)
                                           		<tr>
                                           			<td style="text-align: center;">{{str_pad($row->id, 6, '0' , STR_PAD_LEFT)}}</td>
                                           		<td style="text-align: center;">{{$row->label}}</td>
                                              <td style="text-align: center;">
                                                <div class="form-group">
                                                  <input type="color" class="form-control" disabled value="{{$row->color}}">
                                                </div>
                                              </td>
                                           		<td style="text-align: center;">{{date('d-m-Y',strtotime($row->created_at))}}</td>
                                           		<td style="text-align: center;">
                                           			 <a href="{{route($edit_route,$row->id)}}" class="btn btn-primary btn-circle">
                                                           <i class="fas fa-edit"></i>
                                                        </a>
                                                        <a href="{{route($delete_route,$row->id)}}" class="btn btn-danger btn-circle">
